...
function StudentDetails() {
  ...
  const { student, ready, setAttendance, removeAttendance, toggleBadge } = useStudent(id);
...
<div class="page">
  ...
  return (
    <div className="min-h-screen bg-background pb-12">
      ...
      {/* Badges - manual selection */}
      <section
        className="mx-4 mt-4 rounded-2xl p-4"
        style={{ background: "var(--brand-stats)", boxShadow: "var(--shadow-card)" }}
      >
        <h4 className="mb-1 text-base font-bold text-foreground">🏅 الأوسمة والإنجازات</h4>
        <div className="mb-3 text-xs text-muted-foreground">
          اضغط على الوسام لمنحه أو سحبه — {(student.selectedBadges || []).length} من {BADGES.length}
        </div>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          {BADGES.map((b) => {
            const earned = (student.selectedBadges || []).includes(b.id);
            return (
              <button
                key={b.id}
                type="button"
                onClick={() => {
                  toggleBadge(b.id);
                  toast.success(earned ? "تم سحب الوسام" : "تم منح الوسام");
                }}
                className={`relative overflow-hidden rounded-xl p-3 text-center transition active:scale-95 ${
                  earned ? "text-white" : "bg-card text-muted-foreground opacity-60"
                }`}
                style={earned ? { background: b.color, boxShadow: "var(--shadow-card)" } : undefined}
                title={b.description}
              >
